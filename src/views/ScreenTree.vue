<template>
  <div class="container">
    ...
    <div class="main" :style="{height:mainheight*100+'px'}">
      <vue-draggable-resizable
        v-for="(v,i) in moduleList"
        :key="v.id"
        class-name="chartframe bg-border"
        :w="moduleList[i].width"
        :h="moduleList[i].height"
        :x="moduleList[i].x"
        :y="moduleList[i].y"
        :min-width="50"
        :min-height="50"
        :parent="true"
        :grid="[100,10]"
        @dragstop="onDragstop(0,0,i)"
        @resizestop="onResizestop(0,0,0,0,i)"
      >
        ...
        <div class="chartgroup-content">
          <div v-for="(item,index) in v.graphDataList" :key="item.id" class="charts">
            ...
            <component
              ...
              :index="item.id"
              :size="{width:chartwidth/v.graphDataList.length-22+'px',
                height:chartheight-22+'px'}"
              class="chart"
            ></component>
            <!-- 传入对应的数据给子组件 end -->
...
export default {
  ...
  methods: {
    onResizestop(x, y, width, height,i) {
      console.log(i)
      this.moduleList[i].x = x;
      this.moduleList[i].y = y;
      this.moduleList[i].width = width;
      this.moduleList[i].height = height;
    },
    onDragstop(x, y, i) {
      console.log(x,y,i)
      this.moduleList[i].x = x;
  ...
  computed: {
    chartwidth() {
      return 70;
      // return this.moduleList[i].width;
    },
    chartheight() {
      var heights = 89;
      return heights;
    ...
    title() {
      if (this.$store.state.screenList.length) {
        return this.$store.state.screenList[2].title;
      }
    },
    moduleList() {
      if (this.$store.state.screenList.length) {
        return this.$store.state.screenList[2].moduleList;
      }
    },
    isShow() {
